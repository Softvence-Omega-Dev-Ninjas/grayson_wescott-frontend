...
const BaseCard = ({ item }: IBaseCardProps) => {
  return (
    <div className="flex flex-col items-center justify-center bg-primary-100 py-8 px-5">
      {item.id && (
        <span className="text-white font-medium text-lg mb-3">{item.id}</span>
      )}
      {item.img && (
        <Image
          src={item.img}
      ...
      <h1 className="text-white text-4xl bs:text-3xl xl:text-4xl font-semibold mt-4 text-center">
        {item.title}
      </h1>
      {item.description && (
