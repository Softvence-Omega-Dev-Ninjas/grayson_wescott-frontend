...
export function UserWorkoutCard({ title, description, duration, difficulty, thumbnail, views }: WorkoutCardProps) {
  return (
    <div className="bg-primary-200 border border-secondary p-2 overflow-hidden group ">
      <div className="relative aspect-video">
        ...
        {/* Duration badge */}
        <span className="absolute top-2 left-2 bg-secondary px-3 py-1.5 text-white border-none text-xs font-medium">{duration}</span>
        {/* Difficulty badge */}
        <span className="absolute top-2 right-2 bg-secondary px-3 py-1.5 text-white border-none text-xs font-medium">{difficulty}</span>
        {/* Play overlay */}
      ...
      <div className="p-1">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            <Image src={bodyWeightIcon} alt="Tags" width={8} height={8} className="shrink-0" />
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="font-medium text-sm text-white mt-0.5">{views}</span>
          </div>
        </div>

        {/* Action buttons */}
        <div className="flex gap-2 pt-2">
          <Link href={"/dashboard/user/exercise-library/view-details/1"} className="flex-1 w-full">
            <Button
              variant={"outline"}
              size="sm"
          ...
          <Link href={"/dashboard/user/exercise-library/video/1"} className="flex-1 w-full">
            <Button
              size="sm"
