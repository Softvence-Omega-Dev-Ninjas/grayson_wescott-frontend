    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="h-96 md:h-[450px] overflow-hidden p-0 bg-primary-200 border border-secondary">
        <div className="p-6 overflow-y-auto w-full h-full">
          <DialogHeader>
            <DialogTitle className="text-xl text-light-primary-text dark:text-dark-primary-txt font-bold font-Aclonica">Assign Clients</DialogTitle>
          </DialogHeader>

          {clients.map((client) => (
            <div key={client.id} className="flex items-center justify-between p-3  bg-secondary rounded-none mt-5">
              <div className="flex items-center gap-3">
                <Image src={avatar} width={26} height={26} alt="Upload Icon" className="bg-secondary rounded-full" />
                <div className="flex flex-col">
                  <span className="text-white text-base font-medium">{client.name}</span>
