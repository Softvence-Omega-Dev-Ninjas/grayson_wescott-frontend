...
function ModulePreview() {
  ...
  return (
    <div className="container mx-auto py-5">
        ...
        <SectionHeader title="Module Preview" description="12 comprehensive modules for complete transformation"/> 
        ...
        <div className="max-w-6xl grid mx-auto lg:grid-cols-4 md:grid-cols-3 grid-cols-1 gap-5">
             {
                data.map((item,id)=>(
                 ...
                 <div className="flex flex-col gap-2">
                  <div>
                    <Image alt="image" className="rounded-full" src={item.icon} width={14} height={14}/>
                  </div>
                  <div>
                    <h1 className="text-white font-bold text-xl">{item.title}</h1>
                    <h1 className="text-white/60 font-semibold text-lg">{item.description}</h1>
                  </div>
                 </div>
                </div>
                ))
