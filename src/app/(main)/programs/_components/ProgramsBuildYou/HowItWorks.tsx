...
function HowItWorks() {
  const data = [
    ...
    },
    {
      icon: b3,
      title: "Deload Logic",
      subtitle: "Programmed deloads every 4th week at 60-70% intensity to prevent overreaching",
    },
  ];
  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto  ">
      ...
      <div className="space-y-5">
        {data.map((item, idx) => (
          <div key={idx} className="flex items-center space-x-3 p-5 bg-[#2A2D33] text-white">
            {/* Icon Box */}
            {item.id && (
            ...
            <div className="flex flex-col">
              ...
              <div className="font-normal text-sm text-[#D1D5DB]">{item.subtitle}</div>
            </div>
          </div>
        ))}
      </div>
